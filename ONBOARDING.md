...
#### 手渡しの前に programmatic 完結を尽くす。UI 往復を強いるのは違反（絶対ルール / 2026-09-17 cr/kim 厳命）

cr/kim:「**ユーザーの手間はこの様に最小限になるよう全体の徹底ルールにして。ONBOARDING に反映できない場合は
Kim の Claude Code に依頼してでも全 PC に適用して**」。§1.1 最上位原則と §1.5.1 の**運用強化**。

- **手渡しの前に「自分の tool で終わらせられないか」を必ず尽くす**。API/CLI/MCP/tool（例: cloud routine は
  `RemoteTrigger` で直接作成できる）で完結できるなら、UI で user に押させない。**画面に候補が見えている＝
  user にやらせてよい、ではない**。見えていても自分の tool で作れるなら自分で作る（§1.1.1 の `[手渡し判定]`：
  試した programmatic 経路を3つ以上、結果つきで列挙してからでないと手渡さない）。
- **programmatic 経路が詰まった（例: API が 403）ときは、user を UI に戻さない**。別経路（別実装・別権限・
  **kim の Claude Code へ委譲**）に自分で切り替える。「もう一度この画面を押して」を繰り返させるのは違反。
- どうしても UI 手渡しが要るなら §1.5.1 のとおり **実際に見えている画面のラベルで1クリックずつ**、
  **次の画面は見てから**（1操作 → スクショ受領 → 次の1手）。曖昧な「新しい〇〇を押して」だけは禁止。

**実害（2026-09-17）**: anniversary-mail のフィードバック自動化で、cloud routine 作成に必要な claude.ai↔GitHub 接続を
巡って、cr に「新しいルーティンを押して」等の**曖昧な手順**を出し UI を何度も往復させた（cr「やり方は丁寧に教える
ルールになってない？全然わからない」と2度の不満）。本来は `RemoteTrigger create` で routine を直接作れた／
403 が続くなら user を UI に戻さず別経路へ切り替えるべきだった。§1.5.1・§1.1 の二重違反。
