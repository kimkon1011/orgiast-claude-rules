...
| Drive **file** (view) | `https://drive.google.com/file/d/{ID}/view` | `https://drive.google.com/a/orgiast.jp/file/d/{ID}/view` |
| Drive **folder** | — | `https://drive.google.com/drive/folders/{ID}`（**素URLが正。`/a/orgiast.jp/` は404**） |

末尾に `?authuser=kim@orgiast.jp` を付ける手もあるが、`/a/orgiast.jp/` の方が確実かつ短い。

**⚠ Drive folder だけは `/a/orgiast.jp/` 非対応（重要）**: `https://drive.google.com/a/orgiast.jp/drive/folders/{ID}` は Google が **404**（"The requested URL was not found on this server"）を返す。`/a/{domain}/` の hosted-URL 形式が生きているのは docs.google.com / script.google.com のエディタ系ホストと drive.google.com の `/file/d/` パスだけで、`drive.google.com/a/{domain}/drive/folders/` というルートは存在しない。**folder は素URL `https://drive.google.com/drive/folders/{ID}` で渡す**こと。その上でフォルダ自体を orgiast.jp ドメイン共有（reader）にしておけば、orgiast.jp アカウントでログイン中の相手はそのまま開ける。個人Gmailがデフォルトの相手には「右上アバターで orgiast.jp に切替」を併記する。（2026-06-12 ブース制作アプリ、2026-07-06 AEO設置キット共有で404実測）
