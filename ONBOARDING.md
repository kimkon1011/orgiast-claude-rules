...
**重複は出す前に止める**: `makimono-search.mjs` は公開済みしか検索しないため審査待ち(pending)の同主題を検知できず、出品後の取り下げ経路も無い。そこで `--submit` は出品ログ（pending 含む）と題名を突き合わせ、近似があれば送信せず exit 2 で止める。同主題なら出品しない。別主題だと確認できた時だけ `--force` を足して再実行する。

出品時は**審査待ちを含む近似題名の重複ガード**が働き、近い題名が出品ログにあると送信せず exit 2 で止まる（`makimono-search.mjs` は公開済みしか見ないため、pending の同主題はこのガードでしか止まらない）。同主題なら出品しない。別主題だと確認できたときだけ `--force` を付ける（`--force` でも該当一覧は表示される）。
