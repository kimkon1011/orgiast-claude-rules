...
**手渡すときは必ず response に次の構造化ブロックを書く。これが唯一の通過条件。**
    ...
    **[手渡し判定]**
    ...
    - 目的の代替達成: <目的を手作業なしで満たせないか実際に試した「コマンド → 結果」。無いなら 代替なし(理由: ...)>
...
**1.1.2 手段を先に決めるな。目的を先に書き、目的の代替達成を試してから手渡す（2026-09-22 nishi厳命）**

§1.1.1 の3項目（品質理由・試した経路・却下した経路）は「**その手作業をどう自動化するか**」しか問わない。
だから「手段そのものが不要」という最大の無駄を素通りさせる。**必要性を先に問え。**

- **依頼元が `Claude起案` の手渡しは禁止**。user の依頼に無い副次タスクを Claude が思いつき、
  そのために人を動かすのは、どれだけ丁寧な手順書を付けても違反。自分で完結させるか、落として報告だけにする。
- **目的は手段で書くな**。「gh にログインする」は手段。「このリポにアクセスできるか確かめる」が目的。
  目的の形に直すと、たいてい別経路が見える。
- **目的の代替達成を、手渡す前に実際に1回試す**。試した「コマンド → 結果」を書く。
  ここを飛ばした手渡しは `handoff-quality-gate` が block する。

事故例（2026-09-21）: 「gh CLI にログインして権限照会したい」と考え、OAuth 承認を user に依頼した。
ワンタイムコードは15分で失効するため、5回・75分にわたり人を振り回した。
後から `git ls-remote --heads origin main` を1回打ったら exit 0 で、保存済み資格情報でリポに到達できると分かり、
**gh ログインは最初から不要**だった。しかも user の元の依頼（VS Code の連携エラー）は既に解決済みで、
gh ログインは Claude が勝手に増やした副次タスクだった。3項目は満たしていたのでゲートは毎回 pass した。
<!-- END: 手渡し品質ルール -->
