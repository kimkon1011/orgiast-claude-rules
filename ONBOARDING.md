...
- **ファイル/シートを指し示す時は必ずクリック可能なリンクにする。素のファイルID/ファイル名だけを書かない** (2026-07-23 新設、kim 明示要望「クリックですぐ開けないと逆にこちらに手間がかかる」)。
...
- **⚠️ Fable 5 / Mythos（`model:"fable"`）は使わない**。上位ティアで**別課金**が発生する（2026-07-13 kim 指示「Fable5 はもう使わないように。別料金がかかる」）。設計・横断調査でも Opus/Sonnet で回す。ユーザーが明示的に「Fable5 で」と言った時のみ例外。
...
- 経営司令室の実装で、Codex CLI が未導入という理由だけで従量課金の Sonnet サブエージェント＋Opus 検収を**勝手に選択**した。本来は `npm i -g @openai/codex`（1分）→ login 依頼（user 1クリック）で **ChatGPT 定額枠内・同等以上の品質**で実装できた。kim「**費用対効果の事を第一に考えていない。費用を使わずに効果的な方法があるのに、それを実行せずに、かってに費用がかかってクオリティが低い行動をしている。これは重大な問題です。今後二度と起こらないように**」
...
user がシートのタブを**名前で言及**している場合（例:「下記の『task済シート』に…」）、貼られた URL の `#gid=` パラメータを無条件に信用しない。**URLは古い/別タブを開いていた時にコピーされた可能性が常にある**。

**必須手順**:
1. 対象スプレッドシートの全タブ名一覧を取得する（Sheets API `spreadsheets.get` の `sheets[].properties.title` / `sheetId`、または Drive UI 下部のタブバー）
2. user が言及したタブ名（例:「task済シート」）と、取得した全タブ名を**必ず突き合わせる**
3. URL の gid が指すタブ名と、user が言及した名前が**一致しない場合は、名前が一致する方を正**として使う（URL の gid は無視してよい）
4. Drive の `read_file_content` は大型・複数タブのスプレッドシートに対して **gid を無視し、どのタブかのマーカーも付かない、切り詰められた一部データ**しか返さない（実機で複数回確認済み）。複数タブの大型シートは必ず Sheets API（`spreadsheets.values.get` に `'タブ名'!範囲` を明示）で名前指定して読む
