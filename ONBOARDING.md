...
Claude単独版は写真の見せ方で3回差し戻し、Genspark単独版は写真配置は良いが数字とテキストの見せ方で差し戻し、ハイブリッド v4 で承認された。
実測（写真 / 余白 / 文字量）は Claude単独 3 / 3 / 2、Genspark単独 5 / 4 / 4、ハイブリッド v4 は全項目4〜5。
Claude は構成・原稿・文字と数字の HTML/CSS 組版を担当し、写真の暗化・小配置・スクリーンショット拡大はしない。
Gemini画像モデルは実写真2〜4枚と社長パースから文字なしの大きなビジュアルを合成し、数字・表・図解・文字は作らない。
Genspark AI Slides は写真の面積比・余白・配置文法の参照だけに使い、そのまま採用しない。
提案書・企画書・営業資料・プレゼン・デッキ・スライド・ブースプラン・パース入り資料など、見た目が評価対象になる資料は `/design-deck` を必ず使う。
学会1枚資料のような定型量産物・数表主体の実務資料は対象外で、従来の HTML→PDF を使う。
写真への不満は Gemini で再合成し、文字・数字・表・余白への不満は Claude が `pages.json` / CSS を直す。
写真はまばら・設営中・床にゴミや資材・私的集合写真を除外し、他社ロゴが大きければモザイク、社長パースがあれば AI パースは使わない。
Canva は安定した写真投入経路がないため使わない。詳細: `skills/design-deck/SKILL.md` と `rules-extracted/design-deck-genspark.md`。
