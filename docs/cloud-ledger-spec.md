...
- **カード列(6)・支払い元(7)は機械が書かない。許可リストに入れない。**
- `cloud-contract` は人の列も更新できるが、**非空セルは `force:true` のときだけ上書き**する。
...
- `describeCloudLedger()` … 各タブの **ヘッダ名 / 行数 / キー列の値の一覧のみ** を返す。
  **タブ2の `支払い元カード(下4桁)` と `支払い元(名義)` は応答に含めない**（列名すら値を返さない）。
...
- **禁止列（`支払い元カード(下4桁)` / `支払い元(名義)` / `用途` / `備考` / `状態`）が1つも書かれない**こと。
