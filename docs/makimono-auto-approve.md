...
## §7 残作業（重複ガード用: 審査待ち一覧API）

**背景**: 出品前の重複ガード（`findSimilarPending()` / PR #156）は**そのPCのローカル投稿ログしか見えない**。
既存の `GET /api/v1/listings/{submissionId}` は**単一IDのみ**を返すため、他PC・他セッションが
その日出した審査待ち出品を検知できず、同じ障害・同じ題材を複数PCが独立に出品してしまう
（2026-08-30 実測: 同一障害の言い換えが pending に7件並んだ）。

### 7-1. `GET /api/v1/listings` に一覧モードを追加する

```
GET /api/v1/listings?status=pending
Authorization: Bearer <apiKey>
→ { ok: true, listings: [{ submissionId, title, summary?, category, status, createdAt }, ...] }
```

- 認証は出品時と同じ `Authorization: Bearer <apiKey>`（`mkt_` 信頼済みキーも通常キーも可）
- **そのAPIキーに紐づくメール宛の出品だけ**を返す（他人の出品は返さない。既存の単一ID版と同じ可視範囲）
- 信頼済みキー(`mkt_`)は1メールにつき1本を全PCで共有する運用（`makimono-trusted.env` 経由で配布済み）なので、
  **同じメールの `mkt_` キーを使う複数PCから見れば、これで初めて「今日どのPCが何を出したか」が突き合わせられる**
  （キーがPC単位ではなくメール単位に紐づいている前提を利用する。既存の認証モデルを変えない）
- `status` は省略可（省略時は全件）。`limit`（既定50）・`since`（ISO8601、これ以降の作成分のみ）は任意で足してよい
- 既存の `GET /api/v1/listings/{submissionId}` はそのまま残す（後方互換）

### 7-2. クライアント側（このリポ）の対応 — 本体側の実装後に着手する別タスク

`tools/makimono-publish.mjs` の `findSimilarPending()` に、ローカルログに加えて
`GET /api/v1/listings?status=pending` の結果も候補に含めるよう拡張する。
**このタスクは本体APIが無いと動作確認できないため、7-1 の実装・本番反映を確認してから着手すること。**
先走って実装しない。

### やってはいけないこと（§6-3 と同様の理由）

- 他人（別メール）の出品を一覧に含めること（第三者の未公開情報が漏れる）
- 認証なしでの一覧取得を許すこと
