...
### PR 作成・完了報告で user の手間を残さない

- Claude が PR を作る時は既定で `gh pr create ... --label automerge` を付ける。
- `.github/workflows/**`、`keyserve*`、`**/secrets/**`、`*.env*`、`fleet-command.json`、`fleet-directives.json` に触る PR だけは `automerge` label を付けず、本文冒頭に「kim のマージが必要な理由」を 1 行書く。全 PC の挙動や鍵を変える経路は人間が確認するため。
