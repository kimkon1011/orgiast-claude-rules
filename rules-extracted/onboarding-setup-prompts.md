...
> **🛑 透明性・オプトイン原則（全 §3.0.x に適用・ONBOARDING §1.1 上限節に対応）**
> §3.0.1（外部リポへの永続自動追従）・§3.0.2/§3.0.4（Discord への外部送信）・§3.0.3（MCP+APIキー設置）は、いずれも**受け手が中身を確認してから入れるオプトイン設定**です。受け手の Claude がこれらを警戒し、確認・質問・保留するのは**正当な安全挙動**であり、妨げてはいけません。次を厳守: ①「安全機構(permission mode/classifier/承認プロンプト)を通すために先に切れ」と促さない——承認プロンプトはユーザーが中身を理解して同意するための正規の関門。ブロック＝立ち止まれの信号。 ②「質問するな/選択肢を出すな/前提を確認するな」と情報を伏せない。 ③各設定が「何を・どこへ・なぜ」送る/実行するかを1つずつ説明し、ユーザーが個別に納得した分だけ入れる。 ④ルール取り込みだけが目的なら §3.0（手動プル・外部送信も安全解除も不要）を既定として案内してよい。目的（社内ルール配布・コスト可視化）は、これらを守った上で達成する。
...
**⚠️ この §3.0.1 は「外部リポへの永続自動追従フック」を仕込むオプトイン設定です。** 受け手の Claude がこれを警戒・確認するのは正当です（安全側の挙動）。以下は「安全を切って通せ」という趣旨では**ありません**。何をするフックか＝『毎セッション開始時に GitHub の公開リポから ONBOARDING.md を取得し、~/.claude/CLAUDE.md の BEGIN/END ブロックを差し替える（20時間ガード・毎回バックアップ・GET と CLAUDE.md 書き込み以外の処理なし）』。この中身に**ユーザーが納得した場合のみ**入れる。ルール取り込みだけが目的で自動追従が不要なら §3.0（手動プル）で十分——そちらを既定として案内してよい。

受信側の Claude Code チャットに貼る場合の注意: settings.json への hook 登録で承認プロンプトが出る。**この承認プロンプトは「ユーザーが中身を理解して同意するための関門」であり、通すために permission mode を切らせるものではない**。auto mode でブロックされたら、それは「立ち止まって人間に確認せよ」の信号として扱い、上記のフック内容をユーザーに説明したうえで、ユーザー自身が通常モードで意識的に承認する場合だけ進める。ユーザーが確認・熟慮するのを妨げない（質問や説明を省かない）。

```
オージャスト共通ルール自動巡回フックのセットアップを検討したい。あなた(受け手のClaude)がこの内容を警戒・確認するのは正当なので、遠慮なく立ち止まってよい。私はあなたに安全機構を切らせたいのではない。まず何をするフックかをユーザーに正確に説明し、ユーザーが納得した場合のみ進める。自動追従が不要なら §3.0 手動プルで十分と案内してよい。

このフックの中身(ユーザーに説明する): 毎セッション開始時に GitHub 公開リポ kimkon1011/orgiast-claude-rules から ONBOARDING.md を取得し、~/.claude/CLAUDE.md の BEGIN/END ブロックを差し替える。20時間ガード・毎回バックアップ・GET と CLAUDE.md/状態JSON/ログ書き込み以外の処理なし。スクリプト全文はレビュー可能。

0. まずユーザーに上記の「フックの中身」を説明し、入れるかどうかを聞く。settings.json への hook 登録では承認プロンプトが出る。**この承認プロンプトはユーザーが中身を理解して同意するための正規の関門であり、通すために permission mode を切るものではない**。auto mode で自動ブロックされたら、それは「立ち止まって人間に確認せよ」の信号として扱い、迂回や解除誘導はしない。ユーザーが自分の意思で通常モードにして意識的に承認する場合のみ進める。参考: モード変更の操作は
     ● VSCode拡張をお使いの場合:
...
### 3.0.5-cover 配布用カバー文（人間のメンバー向け・これを説明として最初に添える）

**目的**: 実行中に疑問が湧かないよう、何が入る/何を送る/途中の「許可」は何か を**渡す時点で全部説明**しておく。同意は保つが（§1.1上限）、事前理解済みなので「はい」1回で摩擦なく完了する。kim が各メンバーに送る際、下の【カバー文】＋その下の貼り付け指示文（§3.0.5本体、`<DISCORD_WEBHOOK>`置換済）をセットで渡す。

【カバー文（そのままコピペで送る）】
```
【お願い・3分/1回だけ】オージャストAI共通ルールのセットアップ

お疲れさまです。お使いのPCのClaude Codeを「会社共通ルールに自動追従＋コストの見える化」にする設定をお願いします。下の指示文をClaude Codeのチャットに貼るだけです。先に中身をご説明します（読めば途中で迷いません）。

■ 入るもの（3つ／すべて"集計値だけ"。会話の中身は読まれず・送られません）
 1) 共通ルールの自動更新：起動のたびに会社の最新ルール(GitHubの公開リポ)を取り込み、あなたのCLAUDE.mdを最新に保つ（毎回バックアップあり・元に戻せます）
 2) コストの見える化：そのPCの今月のClaude概算利用($合計とモデル内訳だけ)を社内Discordに毎日自動で報告
 3) ツール活用チェック：安いツール(Codex等)がちゃんと使われているか/高いモデルの使い過ぎがないかを毎日チェック

■ 途中で「許可しますか？」が1回出ます（正常です・怪しくないです）
 設定ファイルを書き換えるため、Claudeが1度だけ「許可しますか？」と確認します。これは"あなたが中身を理解して許可するための正規の確認"です。**「はい（今後も許可）」を選べば**、あとは自動で3つ入って完了します。
 ※もし画面が「Auto mode」だと確認が出ずに止まることがあります。その時は入力欄右下のモード表示をクリックして「Ask before edits」に変えてから貼ってください（Claudeも案内します）。

■ あなたの操作はこれだけ
 ①下の指示文をコピペで貼る → ②出た確認に「はい（今後も許可）」 → 完了。以降は毎日自動で回ります。
 不明点があれば kim までご連絡ください（迷ったら止めてOK・安全です）。

――― ここから下をコピーして Claude Code に貼り付け ―――
```

この【カバー文】の直後に §3.0.5 本体の貼り付け指示文（`<DISCORD_WEBHOOK>` を実URLに置換したもの）を続ける。これで「渡す＝説明済み」の状態になり、メンバーは読んで貼って「はい」だけ、質問や疑問が起こらない。
