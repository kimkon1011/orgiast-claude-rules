...
0. 【最初に必ず実行・ここで一旦停止して待つ】Claudeは自分のpermission modeを確実には判定できないため、無条件で次を user に伝えて返事を待つ:
   「このセットアップは settings.json への hook 登録を1つ含み、auto モードだと承認プロンプトすら出ずブロックされます。お手数ですが先に auto mode を解除してください（貼り付けテキストや私(Claude)側からは切り替えられない仕様です）:
     ・VSCode拡張: 入力ボックス下部のモード表示ラベル（"Auto" 等）をクリック →「Manual」を選択
     ・CLI(ターミナル): Shift+Tab を押してモードを切替（default / Manual にする）
   解除できたら『続けて』と入力してください。既に通常モードなら『続けて』だけでOKです。」
   user から続行の合図が来るまで手順1に進まない。（この後の settings.json 編集は通常モードなら『許可しますか？』プロンプトが出るので、最初のプロンプトで「はい、今後も許可」を選べば以降ほぼ聞かれない。）
