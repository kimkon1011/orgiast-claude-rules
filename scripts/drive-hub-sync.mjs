...
import { getDriveToken, driveApi as api } from '../tools/lib/drive-auth.mjs';
import { findByTitle, listFiles, downloadFileContent, uploadFileContent } from '../tools/lib/drive-files.mjs';
...
if (cmd === 'list') {
  const files = await listFiles(token, parent);
  for (const f of files) console.log(`${f.id}\t${f.mimeType.includes('folder') ? '[dir]' : '     '}\t${f.modifiedTime}\t${f.name}`);
} else if (cmd === 'push') {
  const content = readFileSync(a1);
  const existing = await findByTitle(token, a2, parent);
  if (existing.length > 0) {
    await uploadFileContent(token, { fileId: existing[0].id, content });
    console.log(`updated (id kept): ${a2} ${existing[0].id}${existing.length > 1 ? ` (WARN: ${existing.length} 個 of 同名あり、最新を更新)` : ''}`);
  } else {
    const res = await uploadFileContent(token, { name: a2, parentId: parent, content });
    console.log(`created: ${a2} ${res.id}`);
  }
} else if (cmd === 'pull') {
