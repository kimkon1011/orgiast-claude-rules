...
export function loadPages(){
  ...
  for(const page of pages){
    ...
    ids.add(page.id);
    for(const file of [...(page.photoSources??[]),page.directVisual,page.fallbackPhoto].filter(Boolean)){
...
export function config(){
  ...
  return {title:value.title??'企画書',genImage:local(value.genImage,path.join(root,'gen-image.mjs')),htmlToPdf:local(value.htmlToPdf,path.join(root,'html-to-pdf.mjs')),playwright:value.playwright??'playwright-core'};
}
