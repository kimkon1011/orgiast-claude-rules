...
- 出品（価格は常に 0 = 無料。kim の決定）:
  ...
  - **exit 2 = 近い題名の出品が既にある**（`⚠️ 近い題名の出品が既にあります`）。出品ログの**審査待ち(pending)も含めて**
    題名を突き合わせて止めている（`makimono-search.mjs` は公開済みしか見ないので、pending の同主題はここでしか止まらない）。
    出品後の取り下げ経路は無いので、**同主題なら出品しない**。別主題だと確認できたときだけ `--force` を足して再実行する
    （`--force` でも該当一覧は必ず表示される）。`--dry` でも同じ判定が働く。
