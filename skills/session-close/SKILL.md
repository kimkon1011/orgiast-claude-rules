...
- 重複確認: `node ~/orgiast-claude-rules/tools/makimono-search.mjs "<テーマ>"` で同義の巻物が既にあれば出品しない（既存を更新したい時だけ kim に一言伝える）。
  **search は公開済みしか見ない**ので、審査待ち(pending)の同主題は `--submit` 側の重複ガードが止める（下記）。
...
- 出品（価格は常に 0 = 無料。kim の決定）:
  ...
  - **exit 2 = 近い題名の出品が既にある**場合もある（`⚠️ 近い題名の出品が既にあります`）。出品ログの
    **審査待ち(pending)も含めて**題名を突き合わせて止めている（`makimono-search.mjs` は公開済みしか見ないので、
    pending の同主題はこのガードでしか止まらない）。取り下げ経路が無いので出す前に止めるしかない。
    同主題なら**出品しない**。別主題だと確認できたときだけ `--force` を付けて再実行する（`--force` でも一覧は必ず表示される）。
  - **exit 2 = 近い題名が既に出品済み**（`⚠️ 近い題名の出品が既にあります` が出る）。審査待ちを含む出品ログと
    題名を突き合わせて止めている。**同主題なら出品しない**（取り下げ経路が無いので出した後では戻せない）。
    別主題だと確認できた場合だけ同じコマンドに `--force` を足して再実行する。`--dry` でも同じ判定が働く。
