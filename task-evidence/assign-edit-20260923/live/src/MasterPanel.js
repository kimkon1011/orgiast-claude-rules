...
function _Panel_legacyFeatures() {
  ...
  var features = [
    ...
    { label: '🙋 アサイン依頼を送る (デザイナー/スタッフ/ドライバー手配)', command: 'Phase_AssignRequest_generate', args: [], tasks: '案件概要・日程・必要な人員をまとめた依頼書を作り、アサインチームへ自動で送ります(個別の打診はアサインチームが実施)' },
    ...
    { label: '⚡ 電気申請書類を作成', command: 'Phase2_ElectricalApplication_generate', args: [], tasks: '機材リストから電気容量を計算し、申請記入データ・取付位置図・送付文面を作ります。未知機器や配置は確認シートに回します' },
...
function _Panel_features() {
  var meta = [
    ...
    ['assignRequest','p1','🙋 アサイン依頼を送る (デザイナー/スタッフ/ドライバー手配)'], ['schedule','p1','全体スケジュールを作る'], ['designerBrief','p1','デザイナー依頼文を作る'],
