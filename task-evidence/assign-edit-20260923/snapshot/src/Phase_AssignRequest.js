...
var _ASSIGN_MANUAL_NOTES = [
  ...
  '- 車両の手配方法は「自社トラック」「レンタカー」「JITBOX」を必ず判別して依頼に明記する。判別は現場・搬入出計画・議事録の記載から行い、決められない場合は推測せず「車両手配: 要確認(自社/レンタカー/JITBOX)」と書いて確認事項に上げる。',
  '- JITBOX とレンタカーが両方あり得る場合は JITBOX を優先して明記する(実費・手配先が異なるため)。'
].join('\n');
...
/** 車両手配の記載を、固有名詞の優先順で判別する純粋関数。 */
function AssignRequest_detectVehicleOwnership(text) {
  const normalized = String(text == null ? '' : text).normalize('NFKC').toUpperCase().trim();
  const rules = [
    { type: 'JITBOX', words: ['JITBOX', 'JIT BOX', 'ジットボックス', 'JIT-BOX'] },
    { type: 'レンタカー', words: ['レンタカー', 'レンタル', 'タイムズカー', 'ニコニコレンタカー', 'オリックスレンタカー', 'トヨタレンタカー'] },
    { type: '自社トラック', words: ['自社トラック', '自社車', '自社便', 'デュトロ', 'キャラバン', 'トヨエース', '軽トラ', '2トン', '4トン'] }
  ];
  for (let i = 0; i < rules.length; i++) {
    const evidence = rules[i].words.filter(function (word) { return normalized.indexOf(word) >= 0; });
    if (evidence.length) return { type: rules[i].type, evidence: evidence };
  }
  return { type: '不明', evidence: [] };
}

/** 除外指示に続く対象語だけを読み、元配列を変更せずに返す。 */
function AssignRequest_applyExclusions(rows, inputText) {
  const excluded = {};
  const pattern = /除外[:：]\s*((?:(?:施工スタッフ|イベント関連)(?:[\s、,，]+|$))*(?:施工スタッフ|イベント関連)?)/g;
  const text = String(inputText == null ? '' : inputText);
  let match;
  while ((match = pattern.exec(text)) !== null) {
    const words = match[1].split(/[\s、,，]+/);
    if (words.indexOf('施工スタッフ') >= 0) excluded.construction = true;
    if (words.indexOf('イベント関連') >= 0) excluded.event = true;
  }
  return rows.filter(function (row) { return !excluded[row.type]; });
}

function Phase_AssignRequest_draft(caseId) {
  return Phase_AssignRequest_generate(caseId, { draft: true });
}

function Phase_AssignRequest_sendDraft(caseId) {
  const properties = PropertiesService.getScriptProperties();
  const key = 'ASSIGN_DRAFT_' + caseId;
  const saved = properties.getProperty(key);
  if (!saved) return { ok: false, reason: '下書きがありません。先に「アサイン依頼の下書きを作る」を実行してください' };
  const draft = JSON.parse(saved);
  // 📝欄が取れない状況(パネル外実行・シート名変更等)でも送信は止めない。除外なしとして扱う。
  var inputText = '';
  try {
    var panel = _PanelInput_currentSheet();
    if (panel) inputText = PanelInput_read(panel);
  } catch (e) { inputText = ''; }
  const rows = AssignRequest_applyExclusions(draft.rows, inputText);
  if (!rows.length) return { ok: false, reason: '送信対象の行がありません(除外指定を確認してください)' };
  _AssignSheet_ensureDocUrlHeader();
  const insertedRows = _AssignSheet_insertRowsTop(rows.map(function (row) {
    const values = row.values.slice();
    // JSON 保存で文字列になった依頼日時をシートの日付型に戻す。
    values[0] = new Date(values[0]);
    return values;
  }));
  const appendedRows = rows.map(function (row, index) { return { type: row.type, row: insertedRows[index] }; });
  const revised = Boolean(draft.sentAt);
  const notifyText = _AssignSheet_buildNotifyText({
    roleId: draft.recipientRoleId,
    requesterName: draft.requesterName,
    requestTypes: rows.map(function (row) { return row.type === 'construction' ? '施工スタッフ' : 'イベント関連'; }),
    ...
    producerMissing: draft.requesterName === 'ブース制作アプリ(自動)'
  });
  const discordResult = _AssignSheet_notifyDiscord((revised ? '【修正版】\n' : '') + notifyText);
  // シート登録済みなら通知失敗時も記録し、次回を修正版として扱う。
  draft.sentAt = new Date().toISOString();
  properties.setProperty(key, JSON.stringify(draft));
  MasterWriteBack_recordArtifact(caseId, 'アサイン依頼', draft.title, draft.docUrl);
  return {
    ok: true, appendedRows: appendedRows, assignSheetUrl: _AssignSheet_masterUrl(appendedRows[0].row),
    docUrl: draft.docUrl, discordSent: discordResult.sent, discordReason: discordResult.reason, revised: revised
  };
}

function Phase_AssignRequest_discardDraft(caseId) {
  PropertiesService.getScriptProperties().deleteProperty('ASSIGN_DRAFT_' + caseId);
  return { ok: true };
}
...
function Phase_AssignRequest_generate(caseId, opts) {
  ...
  const draft = Boolean(opts && opts.draft === true);
  ...
  if (draft) {
    const properties = PropertiesService.getScriptProperties();
    properties.setProperty('ASSIGN_DRAFT_' + caseId, JSON.stringify({
      caseId: caseId, createdAt: new Date().toISOString(), title: title, docUrl: docUrl,
      projectCode: projectCode, clientName: c.clientName || '', caseName: c.caseName || '',
      requesterName: requesterName, rows: plannedRows,
      recipientRoleId: properties.getProperty('DISCORD_ASSIGN_ROLE_ID') || _ASSIGN_ROLE_ID_DEFAULT,
      sentAt: null
    }));
  }
  if (!draft && confirmations.length > 0) {
  ...
  if (draft) {
    ...
    result.plannedRows = plannedRows;
    result.panelStatus = '📝 アサイン依頼の下書きを保存しました。依頼書と📝欄を確認して「下書きを確認してアサインチームへ送る」を実行してください';
  }
  return result;
}
