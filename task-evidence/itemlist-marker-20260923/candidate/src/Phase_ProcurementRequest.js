...
function Phase_ProcurementRequest_generate(caseId) {
  ...
  const existingItems = Zissi_readExistingItemList(zissiSs);
...
function _Procure_appendMarkerRow(sheet) {
  try {
    const lastRow = sheet.getLastRow();
    if (lastRow < 3) return 0;
    const isMarker = function (value) {
      let text = String(value);
      if (typeof text.normalize === 'function') text = text.normalize('NFKC');
      return text.replace(/\s/g, '').indexOf('以下に追加') >= 0;
    };
    const columnB = sheet.getRange(1, 2, lastRow, 1).getDisplayValues();
    for (let i = 0; i < columnB.length; i++) {
      if (isMarker(columnB[i][0])) return i + 1;
    }
    const values = sheet.getRange(1, 1, lastRow, sheet.getLastColumn()).getDisplayValues();
    for (let i = 0; i < values.length; i++) {
      if (values[i].some(isMarker)) return i + 1;
    }
  } catch (e) {
    return 0;
  }
...
function _Procure_pickWritableRow(sheet, cols, usedRows) {
  const markerRow = usedRows._procureAppendMarkerRow !== undefined
    ? usedRows._procureAppendMarkerRow
    : (usedRows._procureAppendMarkerRow = _Procure_appendMarkerRow(sheet));
  const firstRow = markerRow > 0 ? markerRow + 1 : 3;
  ...
  if (!usedRows._procureMergedRows) {
    const mergedRows = {};
    const mergedStartRow = Math.max(firstRow, 3);
    if (lastRow >= mergedStartRow) {
      [sheet.getRange(mergedStartRow, 6, lastRow - mergedStartRow + 1, 1), sheet.getRange(mergedStartRow, cols['品名'], lastRow - mergedStartRow + 1, 1)].forEach(function (range) {
        range.getMergedRanges().forEach(function (merged) {
          const mergedFirstRow = Math.max(mergedStartRow, merged.getRow());
          const endRow = Math.min(lastRow, merged.getLastRow());
          for (let row = mergedFirstRow; row <= endRow; row++) mergedRows[row] = true;
        });
      });
    }
  ...
  if (lastRow >= firstRow) {
    const fixed = sheet.getRange(firstRow, 2, lastRow - firstRow + 1, 5).getDisplayValues();
    const names = sheet.getRange(firstRow, cols['品名'], lastRow - firstRow + 1, 1).getDisplayValues();
  ...
  let appendedRow = Math.max(lastRow + 1, firstRow);
