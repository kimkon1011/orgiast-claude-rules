# Claude 利用コスト自動監査 (純PowerShell / LLMトークン不使用)
# 各PCのローカル Claude Code 記録 (~/.claude/projects/**/*.jsonl) を解析し、
# 「開発(プロジェクト)別に どのモデルで 何トークン 推定いくら」+ 設定監査 を算出、
# Discord #claude-code に自動投稿する。人手は「起動」のみ。
#
#   pwsh -File audit.ps1           # 解析してDiscordに投稿
#   pwsh -File audit.ps1 -DryRun   # 投稿せず画面表示のみ(検証用)
#
# 推定$は Anthropic API 定価換算の「目安」(サブスク実額とは別)。開発間の比較・犯人特定用。
