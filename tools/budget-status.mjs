...
export function calculateBudgetStatus({ config, ledgerRows = [], developerPlatform = { available: false, reason: '計測不能' }, now = new Date() }) {
  ...
  return { month: `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`, monthlyBudgetJpy, usdJpy, fixedJpy, fixedItems, unfilled, variableKnownJpy, totalKnownJpy, budgetUsedPct: monthlyBudgetJpy ? totalKnownJpy / monthlyBudgetJpy * 100 : null, projectedJpy, budgetPacePct: monthlyBudgetJpy ? projectedJpy / monthlyBudgetJpy * 100 : null, sources: { developerPlatform, executorUsd, geminiJpy: gemini.totalJpy, geminiUnmeasuredCalls: gemini.unmeasuredCalls, claudeTeamOverage: { available: false, reason: 'APIでは金額取得不能。プラン上限消化率を参照' } } };
}
