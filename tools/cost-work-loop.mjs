...
export function decideEnforcement({ delegRatioWithPrep, delegRatio, linesRatio, daysObserved, claudeOut, history, target, targetLines = 0.50, pilot, previousMode }) {
  ...
  const metricLabel = useLinesRatio ? '行ベース委譲率' : 'トークン委譲率';
  const fallbackNote = useLinesRatio ? '' : ' (行ベース算出不能のためフォールバック)';
  if (!pilot) {
    const demotion = previousMode === 'block' ? '既存blockをwarnへ降格。' : '';
    return { mode: 'warn', reason: `${demotion}block昇格はパイロット機のみ有効(~/.claude/cost-enforce-pilot が無い)。目標50%の指示書と可視化は有効`, decidedBy };
  }

  let mode = 'warn', reason = '観察中';
  if (enforcementRatio < enforcementTarget / 3 && daysObserved >= 2 && claudeOut >= 1e6) {
    mode = 'block'; reason = `${metricLabel}${(enforcementRatio * 100).toFixed(1)}%=目標の1/3未満。2日で昇格${fallbackNote}`;
  } else if (daysObserved >= 3 && claudeOut >= 1e6 && enforcementRatio < enforcementTarget / 2) {
    mode = 'block'; reason = `${metricLabel}${(enforcementRatio * 100).toFixed(1)}%=目標の半分未満。3日でトレンドに関わらず昇格${fallbackNote}`;
  } else if (daysObserved >= 7 && claudeOut >= 1e6 && enforcementRatio < enforcementTarget) {
    const avg = a => {
      const values = useLinesRatio ? a.map(x => x.linesRatio).filter(x => typeof x === 'number' && Number.isFinite(x)) : a.map(x => x.delegRatioWithPrep ?? x.delegRatio ?? 0);
    ...
    if (avg(recent) <= avg(early) + 0.05) {
      mode = 'block'; reason = `${daysObserved}日観察して${metricLabel}が改善せず(${(avg(early) * 100).toFixed(0)}%→${(avg(recent) * 100).toFixed(0)}%)。ハードブロック昇格${fallbackNote}`;
    } else {
      reason = `${metricLabel}に改善傾向あり(${(avg(early) * 100).toFixed(0)}%→${(avg(recent) * 100).toFixed(0)}%)=警告継続${fallbackNote}`;
    }
  }
...
else if (claudeOut >= 1e6 && !(typeof linesRatio === 'number' && Number.isFinite(linesRatio)) && delegRatio < TARGET_DELEG) flags.push(`🚨 委譲不足: Claude出力${(claudeOut / 1000).toFixed(0)}k tokなのにトークン委譲率${(delegRatio * 100).toFixed(0)}%(目標${TARGET_DELEG * 100}%↑、行ベース算出不能のためフォールバック)。実装→Codex/量産→Groq/汎用安→OpenRouter/中量級→Kimi K3 へ回す`);
