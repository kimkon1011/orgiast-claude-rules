...
test('line ratio above target remains warn despite low token ratio', () => {
...
test('line ratio below threshold blocks after 3 days', () => {
  const result = decideEnforcement({ ...base, linesRatio: 0.10, daysObserved: 3, pilot: true });
  assert.equal(result.mode, 'block');
});

test('null line ratio falls back to token ratio', () => {
...
test('seven-day trend reads line ratio history', () => {
