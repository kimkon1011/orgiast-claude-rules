...
async function runOne(name, model, cfg = {}) { let tasks = readJsonl(TASKS); const cat = opt('--category'); if (cat) tasks = tasks.filter((t) => t.category === cat || t.category.startsWith(cat)); const limit = Number(opt('--limit', '0')); if (limit > 0) tasks = tasks.slice(0, limit); if (!tasks.length) throw new Error('対象タスクがありません'); const cliConcurrency = opt('--concurrency', ''), concurrency = Math.max(1, Number(cliConcurrency || cfg.concurrency || 1) || 1), rows = new Array(tasks.length); let cursor = 0; async function worker() { for (;;) { const i = cursor++; if (i >= tasks.length) return; const task = tasks[i], started = new Date().toISOString(); let out = null, judgeVerdict = null; try { out = await call(name, model, task.prompt, task.system || '', task.max || 512, cfg); const costUsd = (out.inTok * PRICE[name][0] + out.outTok * PRICE[name][1]) / 1e6; if (out.truncated) { rows[i] = { t: started, id: task.id, category: task.category, status: 'truncated', pass: false, output: out.text.slice(0, 2000), expect: task.expect, judgeVerdict, error: null, finishReason: out.finishReason, inTok: out.inTok, outTok: out.outTok, costUsd, ms: out.ms, judgeUsage: null }; console.log(`切断 ${task.id} in=${out.inTok} out=${out.outTok} $${costUsd.toFixed(6)} ${out.ms}ms`); continue; } let pass, judgeUsage = null; if (task.expect.type === 'judge') { const judged = await judge(task, out.text); pass = judged.pass; judgeVerdict = judged.verdict.slice(0, 2000); judgeUsage = { provider: opt('--judge-provider', 'groq'), inTok: judged.usage.inTok, outTok: judged.usage.outTok, ms: judged.usage.ms }; } else pass = machine(task.expect, out.text); const status = pass ? 'pass' : 'fail'; rows[i] = { t: started, id: task.id, category: task.category, status, pass, output: out.text.slice(0, 2000), expect: task.expect, judgeVerdict, error: null, finishReason: out.finishReason, inTok: out.inTok, outTok: out.outTok, costUsd, ms: out.ms, judgeUsage }; console.log(`${status.toUpperCase()} ${task.id} in=${out.inTok} out=${out.outTok} $${rows[i].costUsd.toFixed(6)} ${out.ms}ms`); } catch (e) { const costUsd = out ? (out.inTok * PRICE[name][0] + out.outTok * PRICE[name][1]) / 1e6 : 0; rows[i] = { t: started, id: task.id, category: task.category, status: 'error', pass: false, output: out?.text.slice(0, 2000) || '', expect: task.expect, judgeVerdict, error: String(e.message || e).slice(0, 400), finishReason: out?.finishReason ?? null, inTok: out?.inTok || 0, outTok: out?.outTok || 0, costUsd, ms: out?.ms || 0, judgeUsage: null }; console.error(`ERROR ${task.id}: ${e.message}`); } } }
