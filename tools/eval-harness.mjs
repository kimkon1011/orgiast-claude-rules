...
import { activeProviderCooldown, nextUtcDay, setProviderCooldown } from './provider-cooldown.mjs';
...
function scheduler(name, minIntervalMs) { if (!schedulers.has(name)) schedulers.set(name, { next: 0, chain: Promise.resolve() }); const s = schedulers.get(name); return async () => { let release; const prior = s.chain; s.chain = new Promise((resolve) => { release = resolve; }); await prior; const wait = Math.max(0, s.next - Date.now()); if (wait) await sleep(wait); s.next = Date.now() + minIntervalMs; release(); }; }
function retryDelay(response, retryIndex, now = Date.now()) { const h = response.headers.get('retry-after'); if (h) { const seconds = Number(h); if (Number.isFinite(seconds)) return Math.max(0, seconds * 1000); const date = Date.parse(h); if (Number.isFinite(date)) return Math.max(0, date - now); } return 1000 * (2 ** retryIndex); }
export const DAILY_QUOTA_RE = /(?=.*\b(?:RPD|TPD)\b)(?=.*\bper\s+day\b)/is;
function cooldownMessage(name, state, now = Date.now()) { const minutes = Math.max(1, Math.ceil((Number(state.until) - now) / 60000)); return `SKIP ${name}: cooldown中 (${state.reason || 'unknown'}, 残り${minutes}分)`; }
export async function call(name, model, prompt, system, max, runtime = {}) { const P = PROVIDERS[name], now = runtime.now?.() ?? Date.now(), cooldown = activeProviderCooldown(name, { cooldownFile: runtime.cooldownFile, now }); if (cooldown) throw new Error(cooldownMessage(name, cooldown, now)); const key = loadKey(name); if (!key) throw new Error(`${P.keyEnv} 未設定。環境変数または ~/.claude/${P.keyFile} に ${P.keyEnv}=値 を置いてください`); let body, headers; if (P.anthropic) { body = { model, max_tokens: max, messages: [{ role: 'user', content: prompt }] }; if (system) body.system = system; headers = { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }; } else { const messages = []; if (system) messages.push({ role: 'system', content: system }); messages.push({ role: 'user', content: prompt }); body = { model, messages, max_tokens: max, stream: false }; headers = { Authorization: `Bearer ${key}`, 'content-type': 'application/json', ...(P.headers || {}) }; }
  ...
  const reserve = scheduler(name, runtime.minIntervalMs || 0), fetchImpl = runtime.fetchImpl || fetch, sleepImpl = runtime.sleepImpl || sleep; let lastError;
  for (let attempt = 0; attempt <= 3; attempt++) { await reserve(); const start = Date.now(); let r; try { r = await fetchImpl(P.base, { method: 'POST', headers, body: JSON.stringify(body), signal: AbortSignal.timeout(runtime.timeoutMs || 120000) }); } catch (e) { throw new Error(`通信エラー: ${e.message}`); } if (!r.ok) { const responseText = await r.text().catch(() => ''), detail = responseText.slice(0, 300); lastError = new Error(`${r.status}: ${detail}`); if (r.status === 400 && paramsApplied && !retriedWithoutParams) { for (const key of Object.keys(params)) delete body[key]; paramsApplied = false; retriedWithoutParams = true; console.error(`RETRY ${name} HTTP 400 paramsなしで再試行`); attempt--; continue; } if (r.status === 429 && DAILY_QUOTA_RE.test(responseText)) { setProviderCooldown(name, { until: nextUtcDay(now), reason: 'daily_quota', at: now }, { cooldownFile: runtime.cooldownFile }); throw lastError; } if ((r.status === 429 || r.status >= 500) && attempt < 3) { const delay = retryDelay(r, attempt, now); console.error(`RETRY ${name} HTTP ${r.status} ${attempt + 1}/3 (${delay}ms後)`); await sleepImpl(delay); continue; } throw lastError; }
