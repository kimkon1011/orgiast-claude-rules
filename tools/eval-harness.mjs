...
function cooldownMessage(name, cooldown, now = new Date()) { const remainingMinutes = Math.max(1, Math.ceil((Number(cooldown.until) - now.getTime()) / 60000)); return `SKIP ${name}: cooldown中 (${cooldown.reason || 'daily_limit'}, 残り${remainingMinutes}分)`; }
export async function call(name, model, prompt, system, max, runtime = {}, dependencies = {}) { const P = PROVIDERS[name], home = dependencies.home || HOME, now = dependencies.now || new Date(), fetchImpl = dependencies.fetchImpl || fetch; const cooldown = activeDailyCooldown(name, { home, now }); if (cooldown) throw new Error(cooldownMessage(name, cooldown, now)); const key = loadKey(name); if (!key) throw new Error(`${P.keyEnv} 未設定。環境変数または ~/.claude/${P.keyFile} に ${P.keyEnv}=値 を置いてください`); let body, headers; if (P.anthropic) { body = { model, max_tokens: max, messages: [{ role: 'user', content: prompt }] }; if (system) body.system = system; headers = { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }; } else { const messages = []; if (system) messages.push({ role: 'system', content: system }); messages.push({ role: 'user', content: prompt }); body = { model, messages, max_tokens: max, stream: false }; headers = { Authorization: `Bearer ${key}`, 'content-type': 'application/json', ...(P.headers || {}) }; }
  ...
  for (let attempt = 0; attempt <= 3; attempt++) { await reserve(); const start = Date.now(); let r; try { r = await fetchImpl(P.base, { method: 'POST', headers, body: JSON.stringify(body), signal: AbortSignal.timeout(runtime.timeoutMs || 120000) }); } catch (e) { throw new Error(`通信エラー: ${e.message}`); } if (!r.ok) { const detail = (await r.text().catch(() => '')).slice(0, 300); lastError = new Error(`${r.status}: ${detail}`); if (r.status === 400 && paramsApplied && !retriedWithoutParams) { for (const key of Object.keys(params)) delete body[key]; paramsApplied = false; retriedWithoutParams = true; console.error(`RETRY ${name} HTTP 400 paramsなしで再試行`); attempt--; continue; } const retryMs = retryDelay(r, attempt); if (isDailyQuotaResponse(r.status, detail, retryMs)) { markDailyProviderCooldown(name, { home, now: dependencies.now || new Date() }); throw lastError; } if ((r.status === 429 || r.status >= 500) && attempt < 3) { const delay = retryMs; console.error(`RETRY ${name} HTTP ${r.status} ${attempt + 1}/3 (${delay}ms後)`); await sleep(delay); continue; } throw lastError; }
...
for (const x of targets) { if (!PROVIDERS[x.provider]) { console.log(`SKIP ${x.provider}: 未対応provider`); continue; } if (x.skip) { console.log(`SKIP ${x.provider}: モデル未導入のためスキップ`); continue; } const cooldown = activeDailyCooldown(x.provider, { home: HOME }); if (cooldown) { console.log(cooldownMessage(x.provider, cooldown)); continue; } if (!loadKey(x.provider)) { const P = PROVIDERS[x.provider]; console.log(`SKIP ${x.provider}: ${P.keyEnv} 未設定（env または ~/.claude/${P.keyFile}）`); continue; } const model = x.model || PROVIDERS[x.provider].model; if (x.provider === 'ollama' && (!model || !(await ollamaHasModel(model)))) { console.log('SKIP ollama: モデル未導入のためスキップ'); continue; } try { await runOne(x.provider, model, x); } catch (e) { console.error(`${x.provider} 実行失敗: ${e.message}`); if (!has('--all')) process.exitCode = 1; } }
if (has('--all')) {
