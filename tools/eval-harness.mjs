...
function dailyCooldownSkip(name) {
  const now = new Date();
  const cooldown = activeDailyCooldown(name, { home: HOME, now });
  return cooldown ? `SKIP ${name}: cooldown中 (${cooldown.reason}, 残り${Math.ceil((cooldown.until - now.getTime()) / 60000)}分)` : null;
}
// isDailyLimit: 本文の RPD 系文字列だけで日次上限を判定する公開API(単体テスト・外部呼び出し用)。
// 実際の call() 内の一次判定は isDailyQuotaResponse(retry-after長さのフォールバックつき)を使う。
export function isDailyLimit(status, detail) { return isDailyQuotaResponse(status, detail); }
// retryDelay: 通常リトライの待ち時間(公開・30秒クランプ)。日次上限フォールバック判定には
// クランプ前の生の retry-after 長さが要る(rawRetryAfterMs)ため、判定用途では使わない。
export function retryDelay(response, retryIndex) { const h = response.headers.get('retry-after'); let delay; if (h) { const seconds = Number(h); if (Number.isFinite(seconds)) delay = Math.max(0, seconds * 1000); else { const date = Date.parse(h); delay = Number.isFinite(date) ? Math.max(0, date - Date.now()) : 1000 * (2 ** retryIndex); } } else delay = 1000 * (2 ** retryIndex); return Math.min(delay, 30000); }
function rawRetryAfterMs(response, retryIndex) { const h = response.headers.get('retry-after'); if (h) { const seconds = Number(h); if (Number.isFinite(seconds)) return Math.max(0, seconds * 1000); const date = Date.parse(h); if (Number.isFinite(date)) return Math.max(0, date - Date.now()); } return 1000 * (2 ** retryIndex); }
export const JUDGE_CHAIN = ['groq', 'deepseek', 'openrouter', 'gemini', 'anthropic'];
export function resolveJudgeChain(preferred, chain = JUDGE_CHAIN) { const p = (preferred || '').toLowerCase(); const rest = chain.filter((x) => x !== p); return p ? [p, ...rest] : [...chain]; }
async function main() {
...
async function call(name, model, prompt, system, max, runtime = {}) { const skip = dailyCooldownSkip(name); if (skip) { const err = new Error(skip); err.dailyLimit = true; err.provider = name; throw err; } const P = PROVIDERS[name], key = loadKey(name); if (!key) throw new Error(`${P.keyEnv} 未設定。環境変数または ~/.claude/${P.keyFile} に ${P.keyEnv}=値 を置いてください`); let body, headers; if (P.anthropic) { body = { model, max_tokens: max, messages: [{ role: 'user', content: prompt }] }; if (system) body.system = system; headers = { 'x-api-key': key, 'anthropic-version': '2023-06-01', 'content-type': 'application/json' }; } else { const messages = []; if (system) messages.push({ role: 'system', content: system }); messages.push({ role: 'user', content: prompt }); body = { model, messages, max_tokens: max, stream: false }; headers = { Authorization: `Bearer ${key}`, 'content-type': 'application/json', ...(P.headers || {}) }; }
  ...
  for (let attempt = 0; attempt <= 3; attempt++) { await reserve(); const start = Date.now(); let r; try { r = await fetch(P.base, { method: 'POST', headers, body: JSON.stringify(body), signal: AbortSignal.timeout(runtime.timeoutMs || 120000) }); } catch (e) { throw new Error(`通信エラー: ${e.message}`); } if (!r.ok) { const detail = (await r.text().catch(() => '')).slice(0, 300); lastError = new Error(`${r.status}: ${detail}`); if (r.status === 400 && paramsApplied && !retriedWithoutParams) { for (const key of Object.keys(params)) delete body[key]; paramsApplied = false; retriedWithoutParams = true; console.error(`RETRY ${name} HTTP 400 paramsなしで再試行`); attempt--; continue; } const rawMs = rawRetryAfterMs(r, attempt); if (isDailyQuotaResponse(r.status, detail, rawMs)) { markDailyProviderCooldown(name, { home: HOME }); const err = new Error(`${r.status}: ${detail}`); err.dailyLimit = true; err.provider = name; throw err; } if ((r.status === 429 || r.status >= 500) && attempt < 3) { const delay = retryDelay(r, attempt); console.error(`RETRY ${name} HTTP ${r.status} ${attempt + 1}/3 (${delay}ms後)`); await sleep(delay); continue; } throw lastError; }
...
const judgeExhausted = new Set(); let activeJudge = null; function judgeCandidates() { const req = (opt('--judge-provider', '') || '').toLowerCase(); if (req && !PROVIDERS[req]) throw new Error(`採点provider不明: ${req}`); return resolveJudgeChain(req, JUDGE_CHAIN).filter((p) => PROVIDERS[p] && loadKey(p)); }
function judgesExhaustedError() { const e = new Error('採点provider全滅: 日次上限'); e.judgesExhausted = true; return e; }
async function judge(task, answer) {
  if (!activeJudge) { activeJudge = judgeCandidates().filter((p) => !judgeExhausted.has(p))[0]; if (!activeJudge) throw judgesExhaustedError(); }
  for (;;) {
    const jp = activeJudge, P = PROVIDERS[jp]; if (!P) throw new Error(`採点provider不明: ${jp}`);
    const cfg = providerConfig(jp);
    const prompt = `次の回答を評価基準だけに照らして二択で判定してください。判定不能という回答は禁止です。最終行に必ず PASS または FAIL の1語だけを出力してください。\n評価基準(JSON): ${JSON.stringify(task.expect)}\n回答(JSON): ${JSON.stringify(answer)}`;
    try {
      const x = await call(jp, cfg.model || P.model, prompt, 'あなたは厳格な採点者です。分析後、最終行を必ず PASS または FAIL にしてください。', 1024, cfg);
      const verdict = x.text.trim(); const m = verdict.match(/(?:^|\n)\s*(PASS|FAIL)\s*$/i); if (!m) throw new Error(`judge返答不正: ${verdict.slice(0, 200)}`);
      return { pass: m[1].toUpperCase() === 'PASS', verdict, usage: x, provider: jp };
    } catch (e) {
      if (!e.dailyLimit) throw e;
      judgeExhausted.add(jp);
      const next = judgeCandidates().find((p) => !judgeExhausted.has(p));
      if (!next) throw judgesExhaustedError();
      console.error(`JUDGE切替 ${jp} → ${next} (日次上限)`);
      activeJudge = next;
    }
  }
}
...
async function runOne(name, model, cfg = {}) { let tasks = readJsonl(TASKS); const cat = opt('--category'); if (cat) tasks = tasks.filter((t) => t.category === cat || t.category.startsWith(cat)); const limit = Number(opt('--limit', '0')); if (limit > 0) tasks = tasks.slice(0, limit); if (!tasks.length) throw new Error('対象タスクがありません'); const cliConcurrency = opt('--concurrency', ''), concurrency = Math.max(1, Number(cliConcurrency || cfg.concurrency || 1) || 1), rows = new Array(tasks.length); let cursor = 0, aborted = false; async function worker() { for (;;) { const i = cursor++; if (i >= tasks.length) return; if (aborted) return; const task = tasks[i], started = new Date().toISOString(); let out = null, judgeVerdict = null; try { out = await call(name, model, task.prompt, task.system || '', task.max || 512, cfg); const costUsd = (out.inTok * PRICE[name][0] + out.outTok * PRICE[name][1]) / 1e6; if (out.truncated) { rows[i] = { t: started, id: task.id, category: task.category, status: 'truncated', pass: false, output: out.text.slice(0, 2000), expect: task.expect, judgeVerdict, execDetail: null, error: null, finishReason: out.finishReason, inTok: out.inTok, outTok: out.outTok, costUsd, ms: out.ms, judgeUsage: null }; console.log(`切断 ${task.id} in=${out.inTok} out=${out.outTok} $${costUsd.toFixed(6)} ${out.ms}ms`); continue; } let pass, judgeUsage = null, execDetail = null; if (task.expect.type === 'judge') { const judged = await judge(task, out.text); pass = judged.pass; judgeVerdict = judged.verdict.slice(0, 2000); judgeUsage = { provider: judged.provider, inTok: judged.usage.inTok, outTok: judged.usage.outTok, ms: judged.usage.ms }; } else if (task.expect.type === 'exec_js') { const res = await runCheck(task.expect.check, out.text); pass = res.pass; execDetail = res.detail; } else pass = machine(task.expect, out.text); const status = pass ? 'pass' : 'fail'; rows[i] = { t: started, id: task.id, category: task.category, status, pass, output: out.text.slice(0, 2000), expect: task.expect, judgeVerdict, execDetail, error: null, finishReason: out.finishReason, inTok: out.inTok, outTok: out.outTok, costUsd, ms: out.ms, judgeUsage }; console.log(`${status.toUpperCase()} ${task.id}${execDetail && !pass ? ` (${execDetail})` : ''} in=${out.inTok} out=${out.outTok} $${rows[i].costUsd.toFixed(6)} ${out.ms}ms`); } catch (e) { const costUsd = out ? (out.inTok * PRICE[name][0] + out.outTok * PRICE[name][1]) / 1e6 : 0; rows[i] = { t: started, id: task.id, category: task.category, status: 'error', pass: false, output: out?.text.slice(0, 2000) || '', expect: task.expect, judgeVerdict, execDetail: null, error: String(e.message || e).slice(0, 400), finishReason: out?.finishReason ?? null, inTok: out?.inTok || 0, outTok: out?.outTok || 0, costUsd, ms: out?.ms || 0, judgeUsage: null }; console.error(`ERROR ${task.id}: ${e.message}`); if (!aborted && e.judgesExhausted) { aborted = true; console.error(`中断 ${name}: 採点provider全滅のため残りタスクをスキップ`); } } } }
  await Promise.all(Array.from({ length: Math.min(concurrency, tasks.length) }, worker)); const liveRows = rows.filter(Boolean); const rec = resultRecord(name, model, liveRows); fs.appendFileSync(RESULTS, JSON.stringify(rec) + '\n'); const stamp = rec.t.replace(/[:.]/g, '-'); fs.writeFileSync(path.join(EVAL_DIR, 'runs', `${stamp}-${name}.jsonl`), liveRows.map(JSON.stringify).join('\n') + '\n'); const rate = rec.rate === null ? '計測不能' : `${(rec.rate * 100).toFixed(1)}%`; console.log(`結果 ${name}/${model}: ${rec.pass}/${rec.graded} graded (${rate}) / ${rec.pass}/${rec.n} attempted (${(rec.attemptedRate * 100).toFixed(1)}%) エラー${rec.errors}件 切断${rec.truncated}件 $${rec.costUsd.toFixed(6)} 平均${Math.round(rec.msAvg)}ms`); }
