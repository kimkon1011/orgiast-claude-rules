...
このツールは、あなたのPCのClaudeを次のようにします(すべて自動):
  ...
  3) 開発を安く速くする多数のAIを導入: Codex・Gemini・Manus・DeepSeek・Grok・Ollama(無料)・OpenRouter(413モデル)・Groq(超高速)・Mistral・Kimi
...
# --- OpenRouter / Groq / Mistral / Kimi キー(統合ヘルパー llm-ask.mjs 用) ---
Step "OpenRouter/Groq/Mistral/Kimi の設定 (多モデル・超高速・安コード・別課金プール)"
...
$cbEnv = Join-Path $HOMEDIR '.claude\cerebras.env'
$cbKey = if ($CerebrasKey) { $CerebrasKey } else { $env:ORGIAST_CEREBRAS_KEY }
...
$kimiEnv = Join-Path $HOMEDIR '.claude\kimi-api.env'
if (Test-Path $kimiEnv) { OK "Kimiキー 既存(上書きしません)" }
elseif ($KimiKey) { "MOONSHOT_API_KEY=$KimiKey" | Set-Content -Path $kimiEnv -Encoding UTF8; OK "Kimiキー設定(中量級生成/量産を別課金プールへ委譲可能に)" }
else { Warn "Kimiキー未指定(他機能は動作)" }
...
try {
  ...
  if (Test-Path $nb) {
    . (Join-Path $REPO 'tools\ensure-run-hidden.ps1')
