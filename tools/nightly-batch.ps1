...
try {
    ...
    # 03:20 の auto-session が再開候補を先頭から拾えるよう、auto-close より前に棚卸しする。
    $stalledSessions = Join-Path $PSScriptRoot 'stalled-sessions.mjs'
    if (Test-Path -LiteralPath $stalledSessions -PathType Leaf) {
        $stalledOutput = @(& $node.Source $stalledSessions 2>&1)
        $stalledExit = $LASTEXITCODE
        if ($stalledExit -eq 0) { Write-NightlyLog 'stalled-sessions' 'ok' }
        else {
            Write-NightlyLog 'stalled-sessions' ('error:終了コード' + $stalledExit + ': ' + (Format-NightlyDetail $stalledOutput))
            Write-Warning ("nightly-batch: stalled-sessions exited " + $stalledExit)
        }
    } else { Write-NightlyLog 'stalled-sessions' 'error:ファイルなし' }
