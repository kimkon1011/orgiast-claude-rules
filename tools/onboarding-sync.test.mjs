...
test('zip rollout delivers Gemini pricing config to existing PCs', async () => {
  const f = repositoryFixture();
  fs.mkdirSync(path.join(f.archive, 'config'));
  const price = JSON.stringify({ models: { fixture: { inputUsdPerMillion: 1 } } });
  fs.writeFileSync(path.join(f.archive, 'config', 'gemini-pricing.default.json'), price);
  ...
  assert.equal(fs.readFileSync(path.join(f.repo, 'config', 'gemini-pricing.default.json'), 'utf8'), price);
});
