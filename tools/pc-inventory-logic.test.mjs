...
// 見出しは実シート(PC管理表 2行目)の実表記に合わせる。行番号の列は 'PC No.' ではなく 'No'(2026-08-28 実測)。
const management=['No','ﾉｰﾄor ﾃﾞｽｸﾄｯﾌﾟ'];
...
test('new hostname appends and increments maximum P number',()=>{
  const headers=[...allowed,...management]; const rows=[row(headers,{'No':'P2'}),row(headers,{'No':'P19'})];
  ...
  assert.equal(plan.values[headers.indexOf('No')],'P20'); assert.equal(plan.values[headers.indexOf('ﾉｰﾄor ﾃﾞｽｸﾄｯﾌﾟ')],'ﾉｰﾄ');
});
