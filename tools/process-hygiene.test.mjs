...
import { classify, classifyDetailed, classifyOrphanConsoleWindows, parseOptions, parseProcessLines } from './process-hygiene.mjs';
...
test('閾値は超えた時だけ対象になり上書きできる', () => {
  ...
  assert.deepEqual(parseOptions(['--kill', '--max-age-min', '3', '--max-batch-age-min', '4', '--alert-threshold', '5']), { kill: true, dryRun: false, maxAgeMin: 3, maxBatchAgeMin: 4, alertThreshold: 5 });
});
