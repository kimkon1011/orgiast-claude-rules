...
function usage(message) {
  ...
  console.error('使い方: node tools/session-triage.mjs [--days N|--all|--older-than N] [--top N] [--status 要対応|要確認|完了っぽい|短命/雑談] [--json] [--md path] [--llm] [--provider name] [--all-status] [--include-current] [--include-closed]');
...
const olderThanRaw = option('--older-than');
const topRaw = option('--top');
const days = daysRaw === undefined ? 90 : Number(daysRaw);
const olderThan = olderThanRaw === undefined ? undefined : Number(olderThanRaw);
const top = topRaw === undefined ? 20 : Number(topRaw);
if (!Number.isFinite(days) || days < 0) usage('--days は0以上の数値にしてください');
if (olderThan !== undefined && (!Number.isFinite(olderThan) || olderThan < 0)) usage('--older-than は0以上の数値にしてください');
...
const includeClosed = args.includes('--include-closed') || allStatus;
...
const closedLedgerPath = path.join(os.homedir(), '.claude', 'session-closed-ledger.json');
...
function shorten(s, n) {
  const value = redactSecrets(String(s || '')).replace(/\s+/g, ' ').trim();
...
function analyze(file, headRaw, tailRaw, headEvents, tailEvents, tailBytes) {
  ...
  const displayTitle = redactSecrets(selectedTitle?.[1] || '(タイトルなし)');
...
async function listFiles() {
  ...
  if (!includeClosed) {
    try {
      const ledger = JSON.parse(await fs.readFile(closedLedgerPath, 'utf8'));
      if (ledger && typeof ledger.sessions === 'object' && ledger.sessions !== null) closedSessionIds = new Set(Object.keys(ledger.sessions));
    } catch { /* 台帳が無い/壊れている場合は従来どおり除外しない。 */ }
  }
  ...
  for (const project of projects) {
    ...
    for (const entry of entries) {
      ...
      if (closedSessionIds.has(sessionId)) { stats.skippedFiles++; continue; }
      const filePath = path.join(projectsRoot, project.name, entry.name);
      try {
        const st = await fs.stat(filePath);
        if (olderThan !== undefined && Date.now() - st.mtimeMs < olderThan * DAY_MS) { stats.skippedFiles++; continue; }
...
async function addLlmJudgments(records) {
  ...
  const failRecord = (record, reason) => {
    ...
    console.error(redactSecrets(`llm失敗 ${record.sessionId.slice(0, 8)}: ${detail}`));
  };
  ...
  async function worker() {
    while (cursor < records.length) {
      ...
      let prompt = `あなたは作業状況の判定器。出力はJSON1個のみ、説明禁止。
