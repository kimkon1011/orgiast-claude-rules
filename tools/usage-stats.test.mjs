...
import { calculateDelegation, calculateLinesDelegation, classifyBashCommand, classifyHeadlessSession, collectBashProfile, collectClaudeCostStats, collectClaudeLines, collectClaudeStats, collectCodexOutput, collectLandedLines, collectLedger, collectProviderHealth, collectTurnStats, countPatchLines, estimateSpecAuthoringTokens, extractInlineProgram, formatBashProfile, modelTier, parseCacheStats, resetParseCacheForTests } from './usage-stats.mjs';

test('non-Claude models count in the non-Claude delegation numerator', () => {
  assert.equal(modelTier('glm-5.3'), 'nonclaude');
  assert.equal(modelTier('deepseek-v4-flash'), 'nonclaude');
  const value = calculateDelegation({ byModel: { nonclaude: 40, opus: 60 } });
  assert.equal(value.execOut, 40); assert.equal(value.nonClaudeDelegRatio, .4);
});
...
test('delegation keeps legacy ratio but non-Claude ratio excludes sonnet and haiku', () => { const x = calculateDelegation({ codexOut: 10, execOut: 5, byModel: { sonnet: 20, haiku: 5, opus: 40, fable: 10, default: 10 } }); assert.equal(x.delegated, 40); assert.equal(x.supervisorOut, 60); assert.equal(x.delegRatio, 0.4); assert.equal(x.nonClaudeDelegRatio, 0.15); const moreSonnet = calculateDelegation({ codexOut: 10, execOut: 5, byModel: { sonnet: 120, haiku: 5, opus: 40, fable: 10, default: 10 } }); assert.ok(moreSonnet.nonClaudeDelegRatio < x.nonClaudeDelegRatio); });
test('headless classifier supports explicit marker and historical features', () => { assert.deepEqual(classifyHeadlessSession({ firstUser: '[headless:nightly]\nrun' }), { headless: true, job: 'nightly' }); assert.deepEqual(classifyHeadlessSession({ firstUser: 'あなたは無人で起動された自動セッションです' }), { headless: true, job: 'auto-session' }); assert.deepEqual(classifyHeadlessSession({ cwd: 'C:/x/next-session-launch/work' }), { headless: true, job: 'next-session-launch' }); });
test('headless transcript output is separated by job', () => { const { home, file } = fixture(), now = Date.now(); fs.writeFileSync(file, [JSON.stringify({ timestamp: new Date(now).toISOString(), type: 'user', cwd: '/auto-session', message: { role: 'user', content: '[headless:auto-session]\nrun' } }), JSON.stringify({ timestamp: new Date(now).toISOString(), type: 'assistant', message: { id: 'a', role: 'assistant', model: 'opus', usage: { output_tokens: 42 }, content: [] } })].join('\n')); const x = collectClaudeStats({ home, now }); assert.equal(x.headlessClaudeOut, 42); assert.deepEqual(x.headlessJobs, { 'auto-session': 42 }); });
test('provider health calculates failures, status counts and failover rescues', () => { const { home } = fixture(), now = Date.now(), rows = [{ provider: 'groq', status: 'ok', secs: 1 }, { provider: 'groq', status: 'http_429', secs: 2 }, { provider: 'groq', status: 413, secs: 3 }, { provider: 'openrouter', status: 'ok', failover: true, secs: 4 }].map((x) => ({ ...x, t: new Date(now).toISOString() })); fs.writeFileSync(path.join(home, '.claude', 'executor-usage.jsonl'), rows.map(JSON.stringify).join('\n')); const h = collectProviderHealth({ home, now }); assert.equal(h.providers.groq.failRate, 2 / 3); assert.equal(h.providers.groq.http429, 1); assert.equal(h.providers.groq.http413, 1); assert.equal(h.providers.groq.averageSeconds, 2); assert.equal(h.providers.openrouter.rescuedByFailover, 1); });
