...
const ASSUMPTION_KEYS = ['opsPerMonth', 'manualMinutesPerOp', 'hourlyYen', 'cloudYenPerOp', 'devDayYen'];
const ROUTE_NUMBERS = ['gadgetYen', 'buildDays', 'powerYenPerMonth', 'residualMinutesPerOp'];
const PRICE_LABELS = { primary: '一次情報で確認済み', secondary: '二次情報。公式で要再確認', assumed: '仮定値', 'quote-required': '公開価格なし。見積必須' };
const FLAGS = { '--ops': 'opsPerMonth', '--manual-minutes': 'manualMinutesPerOp', '--hourly-yen': 'hourlyYen', '--cloud-yen-per-op': 'cloudYenPerOp', '--dev-day-yen': 'devDayYen' };
...
export function validateCatalog(catalog) {
  ...
  for (const route of Array.isArray(catalog?.routes) ? catalog.routes : []) {
    ...
    for (const key of ['voiceControl', 'localLLM']) if (typeof route[key] !== 'boolean') errors.push(`${route.id}.${key} は boolean が必要です`);
    if ((route.voiceControl === false || route.localLLM === false) && !nonEmpty(route.requirementFailReason)) errors.push(`${route.id}.requirementFailReason は要件未達の場合に非空文字列が必要です`);
    if (meetsProposalRequirements(route) && Object.hasOwn(route, 'requirementFailReason')) errors.push(`${route.id}.requirementFailReason は要件を満たす場合に指定できません`);
...
export function computeRouteEconomics(route, assumptions, overrides = {}) {
  ...
  const ops = a.opsPerMonth;
  const baselineHoursPerMonth = knownCalculation([ops, a.manualMinutesPerOp], (o, m) => o * m / 60);
  const residualHoursPerMonth = knownCalculation([ops, route.residualMinutesPerOp], (o, m) => o * m / 60);
  ...
  const cloudYenPerOp = route.localLLM === true ? 0 : a.cloudYenPerOp;
  const cloudSpendYenPerMonth = knownCalculation([ops, cloudYenPerOp], (o, y) => o * y);
  // ローカルの支出単価は0でも、削減額には元のクラウド単価が必要。未取得なら純効果までnullを伝播する。
  const cloudSavedYenPerMonth = route.localLLM === true ? knownCalculation([ops, a.cloudYenPerOp], (o, y) => o * y) : 0;
  const monthlyCostYen = knownCalculation([route.powerYenPerMonth, cloudSpendYenPerMonth], (p, c) => p + c);
  const netYenPerMonth = knownCalculation([laborSavedYenPerMonth, cloudSavedYenPerMonth, monthlyCostYen], (l, c, m) => l + c - m);
  ...
  return { ...route, cloudYenPerOp, ops, baselineHoursPerMonth, residualHoursPerMonth, savedHoursPerMonth, laborSavedYenPerMonth, cloudSpendYenPerMonth, cloudSavedYenPerMonth, monthlyCostYen, netYenPerMonth, initialCostYen, paybackMonths, annualNetYen };
}
...
// 提案 P-0136 の必須要件は「音声制御」と「ローカルLLM（クラウドAPI呼び出しの削減）」の両方。
// ESP32-P4 単体はローカルLLMが載らず、クラウド音声APIは従量を減らさないため、
// どちらも純効果が最大でも推奨にしない（要件を満たさない推奨は誤誘導）。
export function meetsProposalRequirements(route) {
  return route.voiceControl === true && route.localLLM === true;
}
...
export function renderMarkdown(catalog, report, assumptions = catalog.assumptions) {
  ...
  const excludedRoutes = catalog.routes.filter((r) => !meetsProposalRequirements(r));
  ...
  const lines = [
    ...
    ...(recommendation ? [
      ...
      `- この推奨は仮定値に依存する（仮定: ${[...ASSUMPTION_KEYS, `${recommendation.id}.residualMinutesPerOp`, ...(recommendation.priceVerify === 'assumed' ? ROUTE_NUMBERS.filter((k) => k !== 'residualMinutesPerOp').map((k) => `${recommendation.id}.${k}`) : [])].join(', ')}）。`,
    ...
    ...(excludedRoutes.length ? [`- 必須要件（音声制御かつローカルLLM）を満たさないため推奨対象外: ${excludedRoutes.map((r) => `${named(r)}＝${r.requirementFailReason}`).join('、')}`] : []), '',
    '## ルート比較', '',
    ...
    '|---|---:|---:|---:|---:|---:|---:|---|---|---|---|',
    ...report.map((r) => `| ${cell(named(r))} | ${display(r.initialCostYen)} | ${display(r.monthlyCostYen)} | ${display(r.savedHoursPerMonth, 2)} | ${display(r.laborSavedYenPerMonth)} | ${display(r.cloudSavedYenPerMonth)} | ${display(r.netYenPerMonth)} | ${payback(r)} | ${PRICE_LABELS[r.priceVerify]} | ${r.voiceControl ? 'あり（台帳）' : 'なし（台帳）'} | ${r.localLLM ? 'あり（台帳）' : 'なし（台帳）'} |`), '',
    ...report.map((r) => `- ${named(r)}: ${r.priceNote} 残作業: ${display(r.residualMinutesPerOp, 2)}分/件（仮定）。${r.residualNote}`), '',
    '## 感度分析', '',
    '| ルート | 50操作/月 | 100操作/月 | 200操作/月 | 400操作/月 |', '|---|---:|---:|---:|---:|',
    ...catalog.routes.map((r) => `| ${cell(named(r))} | ${[50, 100, 200, 400].map((opsPerMonth) => { const e = computeRouteEconomics(r, assumptions, { opsPerMonth }); return e.netYenPerMonth === null ? '-' : display(e.netYenPerMonth); }).join(' | ')} |`), '',
    ...
    ...report.filter((r) => r.priceVerify !== 'quote-required' && (r.monthlyCostYen === null || r.initialCostYen === null)).map((r) => `- ${named(r)}: 見積必須。計算入力に未取得値（null）があり、金額を算出できない。未取得のnullを0円と解釈しない。`),
...
export function runPlan(options = {}) {
  ...
  try {
    ...
    const catalog = JSON.parse(fs.readFileSync(options.catalogPath || path.join(repo, 'tools', 'voice-local-llm-catalog.json'), 'utf8'));
    ...
    const docPath = options.docPath || path.join(repo, 'docs', 'voice-local-llm-plan.md');
    if (config.check) {
      const current = fs.existsSync(docPath) ? fs.readFileSync(docPath, 'utf8').replace(/\r\n/gu, '\n') : null;
      if (current !== markdown) throw new Error('drift: docs/voice-local-llm-plan.md が古い。node tools/voice-local-llm-plan.mjs --write で再生成');
