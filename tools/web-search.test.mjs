...
test('Gemini empty response is still recorded once before fallback, with actual thinking tokens', async () => {
  const rows = [];
  fs.mkdirSync(path.join(isolatedHome, '.claude'), { recursive: true });
  fs.writeFileSync(path.join(isolatedHome, '.claude', 'gemini-pricing.json'), JSON.stringify({ models: { 'gemini-3.6-flash': { inputUsdPerMillion: 1, outputUsdPerMillion: 2 } } }));
  const result = await search('question', {
    provider: 'auto', geminiApiKey: 'test', groqApiKey: 'test', homeDir: isolatedHome,
    appendUsage: (row) => rows.push(row),
    fetchImpl: async (url) => response(url.includes('generativelanguage')
      ? { candidates: [], usageMetadata: { promptTokenCount: 12, candidatesTokenCount: 0, thoughtsTokenCount: 34 } }
      : { choices: [{ message: { content: 'fallback answer' } }], usage: { prompt_tokens: 3, completion_tokens: 4 } }),
  });
  assert.equal(result.provider, 'groq');
  assert.equal(rows.length, 2);
  assert.equal(rows[0].provider, 'gemini'); assert.equal(rows[0].in, 12); assert.equal(rows[0].out, 34);
  assert.equal(rows[0].source, 'web-search'); assert.equal(typeof rows[0].usd, 'number');
});
